...
//	private XMLSerializer xmlSeri;
	...
	public JSONObject myJason(String param){
		...
		addr.append("https://apis.daum.net/local/geo/addr2coord?apikey=").append(apikey);
		addr.append("&q=").append(param).append("&output=json");
//		String address="https://apis.daum.net/local/geo/addr2coord?apikey="+apikey+"&q="+str+"&output=json";
		System.out.println("경로 : "+addr);
		
		try {
			url=new URL(addr.toString().trim());
